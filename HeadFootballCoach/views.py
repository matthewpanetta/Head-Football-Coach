...
#GraduatedAfterSeason
        ...
        if WeekID.UserRecruitingPointsLeftThisWeek > 0:
            UserActions.append({'LinkDisplay': f'Weekly Recruiting ({WeekID.UserRecruitingPointsLeftThisWeek})', 'Href': f'/World/{WorldID}/Recruiting', 'ClassName': ''})

        if not WeekID.LastWeekInPhase:
            UserActions.insert(0,SimAction_Week)

        WeekUpdates = list(WeekID.weekupdate_set.all())
        if len(WeekUpdates) > 0:
            UserActions.append({'LinkDisplay': f'Updates this week ({len(WeekUpdates)})', 'id': 'WeekUpdates', 'Href': '#', 'ClassName': 'week-updates'})
...
def GetRecentGamesForScoreboard(CurrentWorld, CurrentSeason=None, CurrentWeek=None):
    ...
    GameList = Game.objects.filter(WeekID__PhaseID__LeagueSeasonID = CurrentSeason, WorldID = CurrentWorld).values(
        'GameID', 'GameTime'
    ).annotate(
    ...
    TGs = TeamGame.objects.filter(TeamSeasonID__LeagueSeasonID = CurrentSeason, GameID__WeekID = LastWeek).values('TeamSeasonID__TeamID','GameID', 'TeamSeasonID', 'Points', 'TeamRecord', 'TeamSeasonID__TeamID__TeamName', 'TeamSeasonID__TeamID__Abbreviation', 'TeamSeasonID__TeamID__TeamLogoURL').annotate(
    ...
    if CurrentWeek.UserRecruitingPointsLeftThisWeek > 0:
        if RTS.UserRecruitingPointsLeftThisWeek > 0:
            ...
            RTSI = list(RecruitTeamSeasonInterest.objects.filter(RecruitTeamSeasonID = RTS).values(
                ...
                'PlayerRecruitingInterestID__PitchRecruitInterestRank', 'PlayerRecruitingInterestID__TeamInfoTopicID__AttributeName'
            ).annotate(
                KnownPitchRecruitInterestRank = Case(
                    When(PitchRecruitInterestRank_IsKnown = True, then=F('PlayerRecruitingInterestID__PitchRecruitInterestRank')),
                    default=Value(20),
                    output_field=IntegerField()
                ),
                KnownPitchRecruitInterestRank_String = Case(
                    When(KnownPitchRecruitInterestRank = 20, then=Value('Unknown')),
                    When(KnownPitchRecruitInterestRank = 1, then=Value('Most')),
                    When(KnownPitchRecruitInterestRank = 2, then=Value('Very High')),
                    When(KnownPitchRecruitInterestRank = 3, then=Value('Very High')),
                    When(KnownPitchRecruitInterestRank = 4, then=Value('High')),
                    When(KnownPitchRecruitInterestRank = 5, then=Value('High')),
                    When(KnownPitchRecruitInterestRank = 6, then=Value('Medium')),
                    When(KnownPitchRecruitInterestRank = 7, then=Value('Medium')),
                    When(KnownPitchRecruitInterestRank = 8, then=Value('Low')),
                    When(KnownPitchRecruitInterestRank = 9, then=Value('Low')),
                    When(KnownPitchRecruitInterestRank = 10, then=Value('Very Low')),
                    When(KnownPitchRecruitInterestRank = 11, then=Value('Very Low')),
                    When(KnownPitchRecruitInterestRank = 12, then=Value('Least')),
...
def POST_RecruitingCallPitch(request, WorldID, PlayerID, Pitch):
    ...
    if not RTSI.UtilizedThisWeek:
        ...
        return JsonResponse({'message':'Already used this pitch this week'}, status=422)
...
def POST_DiscoverRecruitingPitch(request, WorldID, PlayerID, Pitch):

    print('WorldID, PlayerID, Pitch', WorldID, PlayerID, Pitch)
    ...
    RTSI = RecruitTeamSeasonInterest.objects.filter(RecruitTeamSeasonID__TeamSeasonID__TeamID__IsUserTeam = True, RecruitTeamSeasonID__PlayerTeamSeasonID__PlayerID = PlayerID, PlayerRecruitingInterestID__TeamInfoTopicID__AttributeName = Pitch).annotate(
        ...
        PitchRecruitInterestRank_String = Case(
            When(PlayerRecruitingInterestID__PitchRecruitInterestRank = 1, then=Value('Most')),
            When(PlayerRecruitingInterestID__PitchRecruitInterestRank = 2, then=Value('Very High')),
            When(PlayerRecruitingInterestID__PitchRecruitInterestRank = 3, then=Value('Very High')),
            When(PlayerRecruitingInterestID__PitchRecruitInterestRank = 4, then=Value('High')),
            When(PlayerRecruitingInterestID__PitchRecruitInterestRank = 5, then=Value('High')),
            When(PlayerRecruitingInterestID__PitchRecruitInterestRank = 6, then=Value('Medium')),
            When(PlayerRecruitingInterestID__PitchRecruitInterestRank = 7, then=Value('Medium')),
            When(PlayerRecruitingInterestID__PitchRecruitInterestRank = 8, then=Value('Low')),
            When(PlayerRecruitingInterestID__PitchRecruitInterestRank = 9, then=Value('Low')),
            When(PlayerRecruitingInterestID__PitchRecruitInterestRank = 10, then=Value('Very Low')),
            When(PlayerRecruitingInterestID__PitchRecruitInterestRank = 11, then=Value('Very Low')),
            When(PlayerRecruitingInterestID__PitchRecruitInterestRank = 12, then=Value('Least')),
            default=Value('Unknown'),
            output_field=CharField()
        ),
        KnownPitchRecruitInterestRank_String = Case(
...
def GET_WeekUpdates(request, WorldID):

    CurrentWorld = World.objects.get(WorldID = WorldID)
    CurrentWeek = CurrentWorld.week_set.filter(IsCurrent = True).first()

    WeekUpdates = list(CurrentWeek.weekupdate_set.all().values('MessageText', 'LinkText', 'LinkHref').order_by('-MessageImportanceValue'))

    return JsonResponse({'message': 'This week\'s updates ', 'WeekUpdates': WeekUpdates}, status=200)
...
def POST_ScoutRecruit(request, WorldID, PlayerID):
    ...
    if RTS is not None:
        ...
        return JsonResponse({'message':'No recruit found.'}, status=422)
...
def Page_Conferences(request, WorldID, ConferenceID = None):
    ...
    for CS in ConferenceList:
        ThisConference = {'ConferenceName': CS.ConferenceID.ConferenceName, 'ConferenceAbbreviation': CS.ConferenceID.ConferenceAbbreviation, 'ConferenceID': CS.ConferenceID.ConferenceID, 'ConferenceDivisions': [], 'OpposingConferences': []}
        DivisionsInConference = CS.ConferenceStandings(Small=False,  LeagueSeasonID = CurrentSeason, WeekID = CurrentWeek)

        print('TeamsInConference', DivisionsInConference)
        for d in DivisionsInConference:
            ThisConference['ConferenceDivisions'].append(d)
...
def Page_Schedule(request, WorldID):
    ...
    ShowWeek = Week.objects.filter(PhaseID__LeagueSeasonID = CurrentSeason).annotate(
        GamesPlayed = Count('game__GameID'),
        IsCurrentWeek = Case(
    ...
    PlayerGameStats = PlayerGameStat.objects.filter(PlayerTeamSeasonID__TeamSeasonID__LeagueSeasonID = CurrentSeason, GameScore__gt = 0).select_related('PlayerTeamSeasonID__TeamSeasonID__TeamID', 'TeamGameID__GameID', 'PlayerTeamSeasonID__PlayerID__PositionID', 'PlayerTeamSeasonID__PlayerID').only(
        ...
        AdjustedGameScore = ExpressionWrapper(F('GameScore') * F('GameScoreModifier'), output_field=FloatField()),
        GameScoreRank = Window(
            expression=RowNumber(),
            partition_by=[F('TeamGameID')],
            order_by=F("AdjustedGameScore").desc(),
        ),
        #PlayerGameStatID = F('PlayerGameStatID'),
        TopStatStringDisplay1_Text = Coalesce(F('TopStatStringDisplay1'), Value('')),
        TopStatStringDisplay2_Text = Coalesce(F('TopStatStringDisplay2'), Value('')),
        TopStatStringDisplay3_Text = Coalesce(F('TopStatStringDisplay3'), Value('')),
    ...
    for W in AllWeeks:
        ...
        for G in ThisWeekGames:
            ...
            if G.HomeTeamGameID in PlayerGameStatDict:
                G_Obj['HomeTopPlayers'] = PlayerGameStatDict[G.HomeTeamGameID]
                G_Obj['AwayTopPlayers'] = PlayerGameStatDict[G.AwayTeamGameID]
...
def Page_Awards(request, WorldID, SeasonStartYear = None):
    ...
    for Conf in [None] + ConferenceList:
        ...
        if Conf is None:
            ...
            print('Conf', Conf, [Award for Award in AllAwards if Award['IsConferenceAward'] == True][0])
            ...
            PTSA = [Award for Award in AllAwards if Award['ConferenceID'] == Conf.ConferenceID and Award['IsConferenceAward'] == True]
            for TD in [{'IsFirstTeam': 1, 'IsSecondTeam': 0}, {'IsFirstTeam': 0, 'IsSecondTeam': 1}]:
                T = 'FirstTeam' if TD['IsFirstTeam'] == 1 else 'SecondTeam'
                ShowTeam = '' if T == 'FirstTeam' else 'preseason-allamerican-team-hide'
                ConfDict['Teams'].append({'Team': [Award for Award in PTSA if Award['IsFirstTeam'] == TD['IsFirstTeam'] and Award['IsSecondTeam'] == TD['IsSecondTeam']], 'TeamName': T, 'ShowTeam': ShowTeam})
...
def Page_PlayerStats(request, WorldID, TeamID = None, SeasonStartYear = None):
    ...
    Seasons = [{'SeasonStartYear': 'All-Time', 'SeasonStatsHref': '/World/{WorldID}/PlayerStats/'.format(WorldID=WorldID)}] + list(LeagueSeason.objects.filter(WorldID_id = WorldID).values('SeasonStartYear').annotate(
...
def Page_Player(request, WorldID, PlayerID):
    ...
    PlayerObject = Player.objects.filter(WorldID = CurrentWorld).filter(PlayerID = PlayerID).filter(playerteamseason__TeamSeasonID__LeagueSeasonID__IsCurrent = True).select_related('PositionID')
    FirstPlayerObject = PlayerObject.first()

    CurrentPlayerTeamSeason = FirstPlayerObject.playerteamseason_set.filter(TeamSeasonID__LeagueSeasonID__IsCurrent = True).select_related('TeamSeasonID__TeamID', 'TeamSeasonID__DivisionSeasonID__ConferenceSeasonID__ConferenceID', 'PlayerID__PositionID', 'ClassID').first()
    ...
    PlayerDict = PlayerObject.values('PlayerID', 'PlayerLastName', 'PlayerFirstName', 'PlayerFaceJson','PlayerFaceSVG', 'playerteamseason__ClassID__ClassName','WasPreviouslyRedshirted','JerseyNumber', 'Height', 'Weight', 'CityID', 'PositionID__PositionAbbreviation', 'CityID__CityName', 'CityID__StateID__StateName', 'RecruitingStars', 'IsRecruit', 'RecruitingPointsNeeded').annotate(
...
def GET_PlayerCardInfo(request, WorldID, PlayerID, SeasonStartYear = None):
    ...
    if len(P['PlayerFaceJson']) == 0:
        PlayerObj = Player.objects.filter(WorldID=WorldID, PlayerID=PlayerID).select_related('PositionID').first()
        PlayerFaceJson = GeneratePlayerFaceJSon(PlayerObj)
        PlayerObj.PlayerFaceJson = PlayerFaceJson
        PlayerObj.PlayerFaceSVG = BuildFaceSVG(PlayerFaceJson, TeamJerseyStyle = P['playerteamseason__TeamSeasonID__TeamID__TeamJerseyStyle'], TeamJerseyInvert = P['playerteamseason__TeamSeasonID__TeamID__TeamJerseyInvert'], TeamColors = [P['playerteamseason__TeamSeasonID__TeamID__TeamColor_Primary_HEX'], P['playerteamseason__TeamSeasonID__TeamID__TeamColor_Secondary_HEX']])
        PlayerObj.save()

        P['PlayerFaceSVG'] = PlayerObj.PlayerFaceSVG
    elif P['PlayerFaceSVG'] is None:
        PlayerObj = Player.objects.filter(WorldID=WorldID, PlayerID=PlayerID).select_related('PositionID').first()
        PlayerObj.PlayerFaceSVG = BuildFaceSVG(P['PlayerFaceJson'], TeamJerseyStyle = P['playerteamseason__TeamSeasonID__TeamID__TeamJerseyStyle'], TeamJerseyInvert = P['playerteamseason__TeamSeasonID__TeamID__TeamJerseyInvert'], TeamColors = [P['playerteamseason__TeamSeasonID__TeamID__TeamColor_Primary_HEX'], P['playerteamseason__TeamSeasonID__TeamID__TeamColor_Secondary_HEX']])
        PlayerObj.save()
        P['PlayerFaceSVG'] = PlayerObj.PlayerFaceSVG
...
def GET_RecruitCardInfo(request, WorldID, PlayerID):
    ...
    if len(P['PlayerFaceJson']) == 0:
        ...
        PlayerFaceJson = GeneratePlayerFaceJSon(PlayerObj)
        PlayerObj.PlayerFaceJson = PlayerFaceJson
        PlayerObj.PlayerFaceSVG = BuildFaceSVG(PlayerFaceJson)
        PlayerObj.save()

        P['PlayerFaceSVG'] = PlayerObj.PlayerFaceSVG
    elif P['PlayerFaceSVG'] is None:
        PlayerObj = Player.objects.filter(WorldID=WorldID, PlayerID=PlayerID).select_related('PositionID').first()
        PlayerObj.PlayerFaceSVG = BuildFaceSVG(P['PlayerFaceJson'])
        PlayerObj.save()
        P['PlayerFaceSVG'] = PlayerObj.PlayerFaceSVG
...
def GET_PlayerStats_Player(request, WorldID, PlayerID):
    ...
    CareerStats = Player.objects.filter(PlayerID = PlayerID).values('PlayerID').annotate(  # call `annotate`
            Position = F('PositionID__PositionAbbreviation'),
            GameScore=Sum('playerteamseason__playergamestat__GameScore'),
            ...
            PAS_Sacks=Sum('playerteamseason__playergamestat__PAS_Sacks'),
            PAS_SackYards=Sum('playerteamseason__playergamestat__PAS_SackYards'),
            ...
            SeasonYear = Concat(Min('playerteamseason__TeamSeasonID__LeagueSeasonID__SeasonStartYear'), Value('-'), Max('playerteamseason__TeamSeasonID__LeagueSeasonID__SeasonStartYear'), output_field=CharField()),
            PAS_CompletionPercentage=Case(
                When(PAS_Attempts=0, then=0.0),
                default=(Round(F('PAS_Completions')* 100.0 / F('PAS_Attempts'),1)),
                output_field=FloatField()
...
def Common_PlayerStats( Filters = {}, OrderList = []):
    ...
    Filters['playerteamseason__TeamSeasonID__TeamID__isnull'] = False

    Players = Player.objects.filter(**Filters).values('PlayerID', 'PlayerFirstName', 'PlayerLastName', 'PositionID__PositionAbbreviation', 'playerteamseason__TeamSeasonID__TeamID__TeamName','playerteamseason__TeamSeasonID__TeamID__TeamColor_Primary_HEX', 'playerteamseason__TeamSeasonID__TeamID', 'playerteamseason__TeamSeasonID__TeamID__TeamLogoURL', 'playerteamseason__RedshirtedThisSeason', 'playerteamseason__ClassID__ClassAbbreviation').annotate(
        ...
        playerteamseason__playerteamseasonskill__OverallRating = Max('playerteamseason__playerteamseasonskill__OverallRating'),
        playerteamseason__playerteamseasonskill__Strength_Rating = Max('playerteamseason__playerteamseasonskill__Strength_Rating'),
         playerteamseason__playerteamseasonskill__Agility_Rating = Max('playerteamseason__playerteamseasonskill__Agility_Rating'),
         playerteamseason__playerteamseasonskill__Speed_Rating = Max('playerteamseason__playerteamseasonskill__Speed_Rating'),
         playerteamseason__playerteamseasonskill__Acceleration_Rating = Max('playerteamseason__playerteamseasonskill__Acceleration_Rating'),
         playerteamseason__playerteamseasonskill__Stamina_Rating = Max('playerteamseason__playerteamseasonskill__Stamina_Rating'),
         playerteamseason__playerteamseasonskill__Awareness_Rating = Max('playerteamseason__playerteamseasonskill__Awareness_Rating'),
         playerteamseason__playerteamseasonskill__Jumping_Rating = Max('playerteamseason__playerteamseasonskill__Jumping_Rating'),
         playerteamseason__playerteamseasonskill__ThrowPower_Rating = Max('playerteamseason__playerteamseasonskill__ThrowPower_Rating'),
         playerteamseason__playerteamseasonskill__ShortThrowAccuracy_Rating = Max('playerteamseason__playerteamseasonskill__ShortThrowAccuracy_Rating'),
         playerteamseason__playerteamseasonskill__MediumThrowAccuracy_Rating = Max('playerteamseason__playerteamseasonskill__MediumThrowAccuracy_Rating'),
         playerteamseason__playerteamseasonskill__DeepThrowAccuracy_Rating = Max('playerteamseason__playerteamseasonskill__DeepThrowAccuracy_Rating'),
         playerteamseason__playerteamseasonskill__ThrowOnRun_Rating = Max('playerteamseason__playerteamseasonskill__ThrowOnRun_Rating'),
         playerteamseason__playerteamseasonskill__ThrowUnderPressure_Rating = Max('playerteamseason__playerteamseasonskill__ThrowUnderPressure_Rating'),
         playerteamseason__playerteamseasonskill__PlayAction_Rating = Max('playerteamseason__playerteamseasonskill__PlayAction_Rating'),
         playerteamseason__playerteamseasonskill__PassRush_Rating = Max('playerteamseason__playerteamseasonskill__PassRush_Rating'),
         playerteamseason__playerteamseasonskill__BlockShedding_Rating = Max('playerteamseason__playerteamseasonskill__BlockShedding_Rating'),
         playerteamseason__playerteamseasonskill__Tackle_Rating = Max('playerteamseason__playerteamseasonskill__Tackle_Rating'),
         playerteamseason__playerteamseasonskill__HitPower_Rating = Max('playerteamseason__playerteamseasonskill__HitPower_Rating'),
         playerteamseason__playerteamseasonskill__ManCoverage_Rating = Max('playerteamseason__playerteamseasonskill__ManCoverage_Rating'),
         playerteamseason__playerteamseasonskill__ZoneCoverage_Rating = Max('playerteamseason__playerteamseasonskill__ZoneCoverage_Rating'),
         playerteamseason__playerteamseasonskill__Press_Rating = Max('playerteamseason__playerteamseasonskill__Press_Rating'),
         playerteamseason__playerteamseasonskill__Carrying_Rating = Max('playerteamseason__playerteamseasonskill__Carrying_Rating'),
         playerteamseason__playerteamseasonskill__Elusiveness_Rating = Max('playerteamseason__playerteamseasonskill__Elusiveness_Rating'),
         playerteamseason__playerteamseasonskill__BallCarrierVision_Rating = Max('playerteamseason__playerteamseasonskill__BallCarrierVision_Rating'),
         playerteamseason__playerteamseasonskill__BreakTackle_Rating = Max('playerteamseason__playerteamseasonskill__BreakTackle_Rating'),
         playerteamseason__playerteamseasonskill__Catching_Rating = Max('playerteamseason__playerteamseasonskill__Catching_Rating'),
         playerteamseason__playerteamseasonskill__CatchInTraffic_Rating = Max('playerteamseason__playerteamseasonskill__CatchInTraffic_Rating'),
         playerteamseason__playerteamseasonskill__RouteRunning_Rating = Max('playerteamseason__playerteamseasonskill__RouteRunning_Rating'),
         playerteamseason__playerteamseasonskill__Release_Rating = Max('playerteamseason__playerteamseasonskill__Release_Rating'),
         playerteamseason__playerteamseasonskill__PassBlock_Rating = Max('playerteamseason__playerteamseasonskill__PassBlock_Rating'),
         playerteamseason__playerteamseasonskill__RunBlock_Rating = Max('playerteamseason__playerteamseasonskill__RunBlock_Rating'),
         playerteamseason__playerteamseasonskill__ImpactBlock_Rating = Max('playerteamseason__playerteamseasonskill__ImpactBlock_Rating'),
         playerteamseason__playerteamseasonskill__KickPower_Rating = Max('playerteamseason__playerteamseasonskill__KickPower_Rating'),
         playerteamseason__playerteamseasonskill__KickAccuracy_Rating = Max('playerteamseason__playerteamseasonskill__KickAccuracy_Rating'),
        ClassDisplay = Concat( Max('playerteamseason__ClassID__ClassAbbreviation'), F('PastRedshirtDisplay'), output_field = CharField()),
        PlayerName = Concat(F('PlayerFirstName'), Value(' '), F('PlayerLastName'), output_field=CharField()),
        PlayerHref = Concat(Value('/World/'), Value(WorldID), Value('/Player/'), F('PlayerID'), output_field=CharField()),
        PlayerTeamHref = Concat(Value('/World/'), Value(WorldID), Value('/Team/'), F('playerteamseason__TeamSeasonID__TeamID'), output_field=CharField()),
        GamesPlayed=Sum('playerteamseason__playergamestat__GamesPlayed'),
...
def Common_TeamStats( Filters = {}):
    ...
    Teams = Team.objects.filter(**Filters).values('TeamName','TeamColor_Primary_HEX', 'TeamID', 'TeamLogoURL').annotate(
        ...
        ConferenceID__ConferenceAbbreviation = F('teamseason__DivisionSeasonID__ConferenceSeasonID__ConferenceID__ConferenceAbbreviation'),
        NationalRank = Min('teamseason__teamseasonweekrank__NationalRank'),
        TeamHref = Concat(Value('/World/'), Value(WorldID), Value('/Team/'), F('TeamID'), output_field=CharField()),
        GamesPlayed=Sum('teamseason__teamgame__GamesPlayed'),
        Wins = Count('teamseason__teamgame', filter=Q(teamseason__teamgame__IsWinningTeam = True)),
...
###
        ...
        PNT_AVG = Case(
                            ...
                            default=(Round(F('PNT_Yards')/ F('PNT_Punts'),1)),
                            output_field=FloatField()
                        ),
...
#Get Schedule
    ...
    AllTeams = CurrentSeason.teamseason_set.filter(teamgame__GameID__WasPlayed = True).values('TeamSeasonID', 'TeamID').annotate(
        ...
        PPG = Case(
            ...
            default= Round(F('Points') * 1.0 / F('GP'),1),
            output_field=FloatField()
        ),
        PAPG = Case(
            When(GP = 0, then=Value(0.0)),
    ...
    LastSeason = CurrentSeason.WorldID.leagueseason_set.filter(LeagueSeasonID__lt = CurrentSeason.LeagueSeasonID).order_by('-SeasonStartYear').first()

    RecruitLS = CurrentSeason
    if CurrentWeek.PhaseID.PhaseName == 'Preseason':
        RecruitLS = CurrentSeason.WorldID.leagueseason_set.filter(LeagueSeasonID__lt = CurrentSeason.LeagueSeasonID).order_by('-LeagueSeasonID').first()
    SignedRecruits = RecruitTeamSeason.objects.filter(WorldID = CurrentWorld).filter(TeamSeasonID__TeamID = ThisTeam).filter(TeamSeasonID__LeagueSeasonID = RecruitLS).filter(Signed = True).values(
    ...
    TopPlayersReturning = PlayerTeamSeason.objects.filter(WorldID = CurrentWorld).filter(TeamSeasonID__TeamID = ThisTeam).filter(TeamSeasonID__LeagueSeasonID = CurrentSeason).annotate(
        ...
        PlayerHref = Concat(Value('/World/'), Value(WorldID), Value('/Player/'), F('PlayerID'), output_field = CharField()),
    ).select_related('PlayerID__PositionID', 'ClassID', 'playerteamseasonskill').order_by('-CareerGameScore')[:10]
    ...
    for u in teamgames:
        ...
        GameDict['HomeTeamRankDisplay'] = ''
        GameDict['AwayTeamRankDisplay'] = ''
        if GameDict['HomeTeamRank'] is not None:
        ...
        if GameDict['WasPlayed'] == 1:
            ...
            if GameDict['HomeTeam'].TeamID in AllTeamStats:
                GameDict['HomeTeamStats'] = AllTeamStats[GameDict['HomeTeam'].TeamID]
            if GameDict['AwayTeam'].TeamID in AllTeamStats:
                GameDict['AwayTeamStats'] = AllTeamStats[GameDict['AwayTeam'].TeamID]
...
def Page_Recruiting(request, WorldID, SeasonStartYear = None):
    ...
    for P in Players:
        ...
        if P['playerteamseason__recruitteamseason__IsActivelyRecruiting'] and not P['RecruitSigned']:
