...
const getHtml = async (common) => {
  ...
  var current_week = await db.week
    .where("season")
    .between(season - 1, season + 1)
    .toArray();
  ...
  if (current_week.week_name == "Pre-Season") {
    ...
    let [team_seasons, all_teams, player_team_seasons, all_preseason_awards] = await Promise.all([
      db.team_season
        .where("season")
        .between(common.season - 1, common.season, true, true)
        .and((ts) => ts.team_id > 0)
        .toArray(),
      db.team.toArray(),
      db.player_team_season
        ...
        .between(common.season - 1, common.season, true, true)
        .toArray(),
      db.award.where({week_id: current_week.week_id}).toArray()
    ]);
    ...
    let champion_other_team_seasons = await db.team_season
      .where({ team_id: national_champions_team_season.team_id })
