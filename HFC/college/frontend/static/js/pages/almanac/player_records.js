...
} from "/static/js/modules/football-table/football-table.js";
...
export const page_almanac_player_records = async (common) => {
  ...
  let player_leader_categories = [
    ...
    {
      category_name: "Passing Completions",
      category_abbr: "COMP",
      stat: "timeframe.passing.completions",
    },
    { category_name: "Passing Attempts", category_abbr: "ATT", stat: "timeframe.passing.attempts" },
    {
      category_name: "Passing Yards Per Attempt",
      category_abbr: "YPA",
      stat: "timeframe.passing_yards_per_attempt",
      additional_filter: "timeframe.is_qualified_passer",
    },

    {
      category_name: "Passing Completion Percentage",
      category_abbr: "CMP%",
      stat: "timeframe.completion_percentage",
      additional_filter: "timeframe.is_qualified_passer",
    },
    {
      category_name: "Passer Rating",
      category_abbr: "RAT",
      stat: "timeframe.passer_rating",
      additional_filter: "timeframe.is_qualified_passer",
    },
    {
      category_name: "Passing Yards Per Game",
      category_abbr: "YPG",
      stat: "timeframe.passing_yards_per_game",
    },
    ...
    {
      category_name: "Rushing Yards Per Carry",
      category_abbr: "YPC",
      stat: "timeframe.rushing_yards_per_carry_qualified",
    },

    { category_name: "Rushing Carries", category_abbr: "CAR", stat: "timeframe.rushing.carries" },
    {
      category_name: "Rushing Yards Per Game",
      category_abbr: "YPG",
      stat: "timeframe.rushing_yards_per_game",
    },
    {
      category_name: "Rushes for 20+ Yards",
      category_abbr: "20+",
      stat: "timeframe.rushing.over_20",
    },

    { category_name: "Receiving Yards", category_abbr: "YRD", stat: "timeframe.receiving.yards" },
    {
      category_name: "Receiving Touchdowns",
      category_abbr: "TDs",
      stat: "timeframe.receiving.tds",
    },
    { category_name: "Receptions", category_abbr: "RECs", stat: "timeframe.receiving.receptions" },

    {
      category_name: "Receiving Yards Per Catch",
      category_abbr: "YPC",
      stat: "timeframe.receiving_yards_per_catch_qualified",
    },
    {
      category_name: "Receiving Yards Per Game",
      category_abbr: "YPG",
      stat: "timeframe.receiving_yards_per_game",
    },
    {
      category_name: "Receiving Targets",
      category_abbr: "TARG",
      stat: "timeframe.receiving.targets",
    },

    { category_name: "Points Scored", category_abbr: "Points", stat: "timeframe.games.points" },
    {
      category_name: "Yards From Scrimmage",
      category_abbr: "YRD",
      stat: "timeframe.yards_from_scrimmage",
    },
    ...
    {
      category_name: "Tackles for Loss",
      category_abbr: "TFLs",
      stat: "timeframe.defense.tackles_for_loss",
    },
  ...
  for (const player_leader_category of player_leader_categories) {
    ...
    player_leader_category.stat = original_player_leader_category_stat.replace(
      "timeframe",
      "season_stats"
    );

    var player_team_season_leaders = player_team_seasons.filter(
      (pts) => get(pts, player_leader_category.stat) > 0
    );
    if (player_leader_category.additional_filter) {
      player_team_season_leaders = player_team_season_leaders.filter((pts) =>
        get(pts, player_leader_category.additional_filter.replace("timeframe", "season_stats"))
      );
    }
    player_team_season_leaders = player_team_season_leaders.top_sort(
      5,
      (pts_a, pts_b) =>
        get(pts_b, player_leader_category.stat) - get(pts_a, player_leader_category.stat)
    );

    for (const player_team_season of player_team_season_leaders) {
      let player_team_season_obj = {
        player_team_season: player_team_season,
        value: get(player_team_season, player_leader_category.stat),
      };
      player_leader_category.player_team_seasons.push(player_team_season_obj);
    }

    // player_leader_categories = player_leader_categories.filter(plc => plc.player_team_seasons && plc.player_team_seasons.length > 0);
  
  }

  var record_card = nunjucks_env.renderString(season_html, {
    player_leader_categories: player_leader_categories.filter(plc => plc.player_team_seasons && plc.player_team_seasons.length > 0),
  });
  ...
  $("#nav-game-records-tab").on("click", async function () {
    ...
    const team_games = db.team_game.find();
    ...
    const weeks = db.week.find();
    const weeks_by_week_id = index_group_sync(weeks, "index", "week_id");

    let player_team_games = db.player_team_game.find();
    ...
    for (const player_team_game of player_team_games) {
      player_team_game.player_team_season =
        player_team_seasons_by_player_team_season_id[player_team_game.player_team_season_id];
      ...
      player_team_game.team_game.game.week =
        weeks_by_week_id[player_team_game.team_game.game.week_id];

      player_team_game.team_game.opponent_team_season =
        team_seasons_by_team_season_id[player_team_game.team_game.opponent_team_season_id];
    }
    ...
    for (const player_leader_category of player_leader_categories) {
      ...
      console.log({
        player_leader_category: player_leader_category,
        player_team_games: player_team_games,
      });

      var player_team_game_leaders = player_team_games
        .filter((ptg) => get(ptg, player_leader_category.stat) > 0)
        .top_sort(
          5,
          (ptg_a, ptg_b) =>
            get(ptg_b, player_leader_category.stat) - get(ptg_a, player_leader_category.stat)
        );

      for (const player_team_game of player_team_game_leaders) {
        let player_team_game_obj = {
          player_team_game: player_team_game,
          value: get(player_team_game, player_leader_category.stat),
        };
        player_leader_category.player_team_games.push(player_team_game_obj);
      }

      console.log("player_leader_category", {
        player_leader_category: player_leader_category,
        player_team_season_leaders: player_team_season_leaders,
      });
    }
    ...
    common.render_content.player_leader_categories = player_leader_categories.filter(plc => plc.player_team_games.length > 0);
    var renderedHtml = nunjucks_env.renderString(html, common.render_content);
  ...
  $("#nav-career-records-tab").on("click", async function () {
    ...
    for (const player_leader_category of player_leader_categories) {
      ...
      player_leader_category.season_stat = original_player_leader_category_stat.replace(
        "game_stats",
        "season_stats"
      );
      player_leader_category.stat = original_player_leader_category_stat
        .replace("season_stats", "career_stats")
        .replace("game_stats", "career_stats");
      player_leader_category.players = [];
      player_leader_category.player_team_seasons = [];

      var player_leaders = players
        .filter((p) => get(p, player_leader_category.stat) > 0)
        .top_sort(
          5,
          (p_a, p_b) =>
            get(p_b, player_leader_category.stat) - get(p_a, player_leader_category.stat)
        );

      console.log({
        "player_leader_category.stat": player_leader_category.stat,
        player_leader_category: player_leader_category,
        players: players,
    ...
    var renderedHtml = nunjucks_env.renderString(html, {
      players: players,
      player_leader_categories: player_leader_categories.filter(plc => plc.players.length > 0),
    });

    console.log({
      renderedHtml: renderedHtml,
      html: html,
      "$('#nav-career-records')": $("#nav-career-records"),
    });
