...
def serialize_play(play):
    ...
    if play['qtr'] == '1':
        quarter_seconds_remaining_desc = '1st_qtr'
    elif play['qtr'] == '2':
    ...
    if play['score_differential'] == 'NA':
        score_diff_desc = 'NA'
    elif int(play['score_differential']) > 20:
        score_diff_desc = 'big_leading'
    elif int(play['score_differential']) > 0:
        score_diff_desc = 'leading'
    elif int(play['score_differential']) < -20:
        score_diff_desc = 'big_trailing'
    elif int(play['score_differential']) < 0:
        score_diff_desc = 'trailing'
    else:
        score_diff_desc = 'tie'
    ...
    qtr_desc = 'OT' if int(play['qtr']) > 4 else play['qtr']

    return '|'.join([
        qtr_desc, quarter_seconds_remaining_desc, play['down'], yards_to_go_desc, yard_desc, score_diff_desc
    ])
...
def group_data(plays):
    ...
    grouped_data = {k: v for k, v in sorted(grouped_data.items(), key=lambda item: (item[1].get(
        'run', 0) + item[1].get('pass', 0) + item[1].get('kickoff', 0) + item[1].get('punt', 0), item[0]), reverse=True)}
