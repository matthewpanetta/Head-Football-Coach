...
with open(os.path.join(os.path.dirname(sys.argv[0]), 'play_results.json'), 'r') as r:
    ...
    for filter_name, filter_list in QUERY_CONFIG['filters'].items():
        ...
        verbose_data = [row for row in verbose_data if row.get(
            filter_name) in filter_list]

    if (len(QUERY_CONFIG['sorting']) > 0):
        verbose_data = sorted(verbose_data, key=lambda row: [
                              row[sort_key] for sort_key in QUERY_CONFIG['sorting']])

    for filter_name, filter_list in QUERY_CONFIG['filters'].items():
        QUERY_CONFIG['filters'][filter_name] = list(filter_list)

    QUERY_CONFIG['data'] = verbose_data

    w = open(os.path.join(os.path.dirname(
        sys.argv[0]), 'query_results.json'), 'w')
    w.write(json.dumps(QUERY_CONFIG, indent=2))
    w.close()
